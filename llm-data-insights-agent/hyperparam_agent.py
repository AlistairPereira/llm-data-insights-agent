...
def _load_last_model_report(base_dir: str) -> Optional[Dict[str, Any]]:
    """
    Load the last model_report.json written by model_agent
    for THIS dataset (inside outputs/<dataset_key>/).
    """
    path = os.path.join(base_dir, "model_report.json")
    if not os.path.exists(path):
        print(f"[Hyperparam Agent] No model_report found at {path}.")
        return None
    try:
...
def run_hyperparam_agent(
    ...
    1. Decide dataset-specific output folder.
    2. Load dataset.
    3. Choose / validate target column.
    4. Prepare features + target (same as modeling agent).
    5. If algo_list is not given, read last model_report.json (same dataset)
       to infer which family to tune.
    6. Run hyperparameter experiments.
    7. Save raw JSON results (per dataset).
    8. Ask LLM to explain results and save insights (per dataset).
    """

    print(f"[Hyperparam Agent] Starting hyperparameter search for: {file_path}")

    # 0) Per-dataset output folder
    dataset_key = get_dataset_key_from_path(file_path)  # e.g. "cars", "iris"
    base_dir = os.path.join("outputs", dataset_key)
    os.makedirs(base_dir, exist_ok=True)
    print(f"[Hyperparam Agent] Using output folder: {base_dir}")
    ...
    # 4) If no algo_list → infer from last model_report.json in this dataset folder
    if algo_list is None:
        print("[Hyperparam Agent] No algo_list provided. Trying to infer from last model_agent run...")
        model_report = _load_last_model_report(base_dir)
        if model_report is not None:
            inferred = _infer_algos_from_model_report(model_report, str(problem_type))
            if inferred:
                algo_list = inferred
    ...
    # 6) Save JSON results (per dataset)
    results_path = os.path.join(base_dir, "hyperparam_results.json")
    with open(results_path, "w", encoding="utf-8") as f:
        json.dump(search_results, f, indent=2)
    print(f"[Hyperparam Agent] Raw hyperparameter results saved to {results_path}")
    ...
    insights_path = os.path.join(base_dir, "hyperparam_insights.txt")
    with open(insights_path, "w", encoding="utf-8") as f:
        f.write(insights)
...
if __name__ == "__main__":
    ...
    # Examples:
    #   python hyperparam_agent.py sample_data/cars.csv price
    #       → auto mode (infer algorithms from outputs/cars/model_report.json)
    #
    #   python hyperparam_agent.py sample_data/cars.csv price rf gbr
    #       → manually tune only rf + gbr
    #
