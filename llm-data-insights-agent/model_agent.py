...
def run_model_agent(
    ...
    print(f"[Model Agent] Using output folder: {base_dir}")
    ...
    # save for hyperparam_agent (dataset-specific)
    model_report_path = os.path.join(base_dir, "model_report.json")
    with open(model_report_path, "w", encoding="utf-8") as f:
        json.dump(model_report, f, indent=2)
    print(f"[Model Agent] Model report saved to {model_report_path}")
    ...
    model_insights_path = os.path.join(base_dir, "model_insights.txt")
    with open(model_insights_path, "w", encoding="utf-8") as f:
        f.write(insights)

    print(f"[Model Agent] Model insights saved to {model_insights_path}")
